...
--   --call_gemini  -- replaces: (bayesRisk_le_iInf' hl P π).trans_eq (by simp)
...
--   --call_gemini  -- replaces: the iInf_mul_le_lintegral + gcongr + fun_prop chain
...
--   --call_gemini  -- replaces: rcases isEmpty_or_nonempty Θ / 𝓧 simp blocks
...
--   --call_gemini  -- replaces: rcases isEmpty_or_nonempty 𝓧 <;> simp
...
--   --call_gemini  -- replaces: rcases isEmpty_or_nonempty Θ <;> simp
...
--   --the fun μ hμ ↦ ... lintegral_lintegral_swap block
...
--   intro κ hκ
--   rw [← κ.comp_assoc η]
--   --iInf_le_of_le (κ ∘ₖ η) (iInf_le_of_le inferInstance le_rfl)
